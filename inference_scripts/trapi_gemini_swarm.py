...
from autogen_agentchat.teams import MagenticOneGroupChat, Swarm
...
async def main() -> None:
    ...
    for agent in data["agents"]:
        ...
        system_message = agent["agent_description"] + "Use TERMINATE when whole task is complete."
        # print("System message: ", agent['agent_description'])
        tools = list(set(tool_mapping.get(agent['agent_name'], [])))
    ...
    team = Swarm(
        participants=agent_list, termination_condition=termination
    )
